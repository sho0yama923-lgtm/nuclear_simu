...
現在の主ロードマップは `Simulation Condition Advancement`、current stage は `Stage S7: FEBio-native direct parameter path and load/contact activation validation` です。
...
現在の主ロードマップは S7 です。S1-S4 は contract と serialization の土台として completed、S5-S6 は canonical FEBio-native read success / normal termination まで completed-with-residual です。残っている broad blocker は、既存 path で pressure/contact load は active step から参照されるが `No force acting on the system` と contact pressure all-zero が残ること、そして UI parameter conversion が solver validation の原因切り分けを複雑化していることです。詳細 blocker と直近の切り分けは `PROGRESS.md` に置きます。
...
| Stage S7: FEBio-native direct parameter path and load/contact activation validation | current | define FEBio-native spec as the solver-facing path, export `.feb` without UI parameter conversion, and validate pressure/contact loads through nonzero force transfer |
