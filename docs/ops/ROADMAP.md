...
S7 の目的は、UI parameter -> canonical spec -> FEBio 変換を solver validation の主経路から外し、FEBio-native spec -> CLI/backend export -> FEBio CLI -> converter/import -> diagnostics -> Studio confirmation の direct path で nonzero force transfer を確認できる状態にすることです。S7 内の具体的な3 priority は `PROGRESS.md` に一本化します。
...
現在の主ロードマップは S7 です。S1-S4 は contract と serialization の土台として completed、S5-S6 は canonical FEBio-native read success / normal termination まで completed-with-residual です。残っている blocker は、既存 path で pressure/contact load は active step から参照されるが `No force acting on the system` と contact pressure all-zero が残ること、そして UI parameter conversion が solver validation の原因切り分けを複雑化していることです。
