...
- S8-F で native run diagnostics が native model JSON の `loads.pressure[]` から宣言済み pressure load の面積換算合力を返すようになった。`pipetteSuctionPressureLoadActive=true` は `pipette_suction_surface` に非ゼロの surface pressure load が宣言されていることを示す。一方、`pipetteDirectContactOutputActive=false` は `febio_pipette_cell_contact.csv` / `febio_pipette_contact.csv` / pipette `.xplt` contact force がまだゼロであることを示す。
- S8-G で FEBio log の大文字 warning block (`* WARNING *`) も `warnings.warning` に数えるようにした。FEBioStudio の Job Summary と同じく、normal termination でも stiffness reformation warning があれば `warningFree=false` として扱う。
