...
S7-C 以降、micropipette suction pressure は剛体ピペット面ではなく、変形体側の `pipette_suction_surface` に載せる。本命の物理モデルでは、この surface は nucleus-side capture face を指す。native default では `geometry.pipette.puncture` と `geometry.pipette.tip` を分離し、旧 UI の `xp` 相当は実行接触位置ではなく穿刺 metadata として保持する。確認用 direct case の nucleus-side `pipette_suction_surface` orientation は、FEBio pressure solve で negative jacobian を起こさない `[10,14,15,11]` に固定する。S8-G 以降の outer-cell suction surface は output-channel / Studio winding / contact-transfer を切り分ける診断ブリッジであり、最終の pressure target convention ではない。
