# コードベース構造

この文書は repo 構造と source-of-truth の地図です。入口は [README.md](../README.md)、AI / Codex の運用原則は [AGENT.md](../AGENT.md)、現在状態は [PROGRESS.md](../PROGRESS.md) を参照してください。

## 読む順番

多くの変更では、次の順に読む。

1. [AGENT.md](../AGENT.md)
2. [ACTIVE_FILES.md](../ACTIVE_FILES.md)
3. [docs/CODEBASE_STRUCTURE.md](CODEBASE_STRUCTURE.md)
4. [docs/febio/FEBIO_PATH_OWNERSHIP.md](febio/FEBIO_PATH_OWNERSHIP.md) before touching FEBio export paths
5. [PROGRESS.md](../PROGRESS.md) when touching physics model, main flow, export/import, classification, or proxy/native dependencies

## Active / Legacy FEBio Ownership

Active FEBio export work now lives only in:

- `febio_cases/native/*.native.json`
- `src/febio/native/`
- `scripts/export_febio_native_case.mjs`
- `scripts/dump_native_gmsh_baseline.mjs`
- `scripts/export_febio_from_gmsh_mesh.mjs`
- `scripts/export_febio_from_gmsh_python_api.mjs`
- `tests/febio-native-pipeline.test.mjs`
- `febio_exports/<case>/` generated by that script
- `febio_exports/current_mesh/` generated from the canonical edited Gmsh `.msh`
- `febio_exports/current_mesh_api/` generated from the Python API mesh bridge
- `generated/gmsh_current/mesh.*` generated mesh-edit artifacts
- `docs/febio/GEOMETRY_CONVENTIONS.md`

All previous FEBio export, UI/canonical conversion, public API, browser bridge, import, and classification paths are legacy / compatibility for this purpose. Retired docs and historical generated artifacts are under [legacy/](../legacy/). See [docs/febio/FEBIO_PATH_OWNERSHIP.md](febio/FEBIO_PATH_OWNERSHIP.md).

## 正規情報源の配置

- `febio_cases/native/*.native.json`
  - active FEBio case condition values の source-of-truth。
- `src/febio/native/`
  - active native-only FEBio model expansion、XML serialization、export bundle assembly の source-of-truth。
- `scripts/export_febio_native_case.mjs`
  - active native-only FEBio handoff artifact writer。
- `scripts/dump_native_gmsh_baseline.mjs`
  - active Gmsh rectangular-block mesh artifact writer。正規手編集ファイルは `generated/gmsh_current/mesh.geo`。
- `scripts/export_febio_from_gmsh_mesh.mjs`
  - edited `mesh.msh` を native mesh に round-trip し、`febio_exports/current_mesh/` に FEBio handoff を生成する。
- `scripts/export_febio_from_gmsh_python_api.mjs`
  - generated `mesh.py` を実行し、`mesh.msh` を native mesh に round-trip して `febio_exports/current_mesh_api/` に FEBio handoff を生成する。生成 Python は手編集 source-of-truth ではない。
- `src/model/schema.ts`
  - legacy / compatibility parameter schema、validation、digest、schedule metadata、normalized input spec。
- `src/model/defaults.ts`
  - editable field groups と default values。
- `src/model/types.ts`
  - shared region names、schema constants、coordinate metadata。
- `src/febio/mesh/index.ts`
  - legacy / compatibility shared mesh generation。新しい active native-only solver behavior では `src/febio/native/` を優先する。
- `src/febio/spec/index.ts`
  - legacy / compatibility FEBio-native direct template mapping。新しい active FEBio output path では触らない。
- `src/febio/interfaces/nucleusCytoplasm.ts`
  - legacy / compatibility nucleus-cytoplasm interface helper。active native-only path では `src/febio/native/` の coupling を優先する。
- `src/febio/export/index.ts`
  - legacy / compatibility FEBio template assembly、XML serialization、export bundle entrypoint、detachment event contract metadata、face-data coverage metadata、plotfile `contact traction` bridge contract。
- `src/febio/import/normalizeFebioResult.ts`
  - legacy / compatibility FEBio result normalization entrypoint。active export path では呼ばない。
- `src/results/classification.ts`
  - legacy / compatibility classification と detachment interpretation。active export path では呼ばない。
- `src/public-api.ts`
  - legacy / compatibility public API。active native-only export path では呼ばない。
- `src/browser/main.ts`
  - browser entry。public API を公開し、legacy compatibility scripts を起動する。

## 生成物と派生物

- `generated/dist/`
  - `src/` から `node scripts/build-dist.mjs` で生成される。手で編集しない。
- `generated/gmsh_current/`
  - active Gmsh rectangular-block mesh edit workspace。正規ファイルは `mesh.geo`、`mesh.msh`、`mesh.validation.json`、`mesh.roundtrip.json`、`mesh.source.json`、`mesh.py`。`mesh.geo` は手編集可、`mesh.py` は再生成 artifact。
- `legacy/retired_generated_2026-05-18/generated/febio_exports/`
  - retired legacy / compatibility bridge output、scan、historical run artifacts の退避先。
- `febio_exports/current_mesh/`
  - active edited `.msh` からの FEBio handoff artifact 既定出力先。正規 `.feb` は `current_mesh.feb`。
- `legacy/retired_generated_2026-05-18/febio_exports/`
  - retired versioned FEBio exports and historical solver evidence.
- `tmp/logs/`
  - bridge output logs などの disposable local logs。

## 互換バンドル

- `simulation.js`
  - browser app 用 legacy compatibility layer。schema / classification / detachment の長期 source-of-truth ではない。
- `js/simulation-febio.js`
  - legacy FEBio compatibility layer。
- `js/simulation-ui.js`
  - legacy UI runtime。

Compatibility は退役経路に置く。新しい長期責務は canonical `src/` module に置き、compatibility 側は bridge/caller として薄くする。

## Markdown file role map

Markdown files should have one primary role. If a document no longer owns active guidance, extract durable lessons first, then move it to history or legacy.

### Operating contract

- [AGENT.md](../AGENT.md)
  - Stable AI / Codex operating contract: read order, source-of-truth rules, active-path lock, work granularity, Studio gate, anti-patterns.
- [TASK_REQUEST_TEMPLATE.md](../TASK_REQUEST_TEMPLATE.md)
  - Repeatable task request shape.

### Current state

- [PROGRESS.md](../PROGRESS.md)
  - Current-state file only: active milestone, current facts, interpretation, next bounded task, done condition, important retained findings.
  - Historical detail must be retired when a milestone completes or stops being active.

### Long-term direction

- [docs/ops/ROADMAP.md](ops/ROADMAP.md)
  - Broad roadmap, stage purpose, review gates, later/deferred work, auxiliary roadmap positioning.
  - Fine-grained implementation checklists do not belong here.

### Active ownership and structure

- [ACTIVE_FILES.md](../ACTIVE_FILES.md)
  - Active vs legacy working-surface index.
- [docs/CODEBASE_STRUCTURE.md](CODEBASE_STRUCTURE.md)
  - Repo structure, source-of-truth map, Markdown role map.
- [docs/febio/FEBIO_PATH_OWNERSHIP.md](febio/FEBIO_PATH_OWNERSHIP.md)
  - Active native-only FEBio path vs legacy/compatibility path boundary.

### Active FEBio reference

- [docs/febio/FEBIO_NATIVE_SPEC.md](febio/FEBIO_NATIVE_SPEC.md)
  - FEBio-native spec policy and solver-facing condition ownership.
- [docs/febio/GEOMETRY_CONVENTIONS.md](febio/GEOMETRY_CONVENTIONS.md)
  - Coordinate axes, surface normals, pressure sign, contact-pair convention.
- [docs/febio/FEBIO_OUTPUT_MAPPING.md](febio/FEBIO_OUTPUT_MAPPING.md)
  - Logfile / `.xplt` / Studio output channel mapping and parser caveats.
- [docs/febio/NATIVE_MODEL_REFINEMENT_STRATEGY.md](febio/NATIVE_MODEL_REFINEMENT_STRATEGY.md)
  - Active native model refinement strategy. Keep it as strategy; move incidents and long run logs to the relevant ops/history docs.

### Problem-solving and confirmation gates

- [docs/ops/INCIDENTS_AND_ROOT_CAUSES.md](ops/INCIDENTS_AND_ROOT_CAUSES.md)
  - Curated stock of major problem causes, prevention rules, reusable failure patterns, and troubleshooting order.
- [docs/ops/STUDIO_CONFIRMATION_GATES.md](ops/STUDIO_CONFIRMATION_GATES.md)
  - What the agent can verify mechanically and what requires FEBio Studio human confirmation.

### Temporary active diagnostics

- [docs/febio/CELL_DISH_LOAD_BEARING_DIAGNOSTICS.md](febio/CELL_DISH_LOAD_BEARING_DIAGNOSTICS.md)
  - Temporary S7-K diagnostic definition for cell-dish load-bearing gates.
  - When S7-K closes, either merge durable output-channel content into `FEBIO_OUTPUT_MAPPING.md` and root-cause lessons into `INCIDENTS_AND_ROOT_CAUSES.md`, or explicitly mark this file historical.

### Archived history

These files are historical records and are not current source-of-truth. Durable lessons from them should live in active docs above.

- `docs/history/febio/PRESSURE_SUCTION_STAGE_S3.md`
- `docs/history/febio/ASPIRATION_OUTPUT_STAGE_S4.md`
- `docs/history/febio/STICKY_COHESIVE_STAGE_S5.md`
- `docs/history/febio/TRUE_COHESIVE_STAGE_S6.md`
- `docs/history/2026-04-progress-history.md`

### Already legacy

- `legacy/docs/febio/*`
- `legacy/febio_exports/*`
- `legacy/README.md`

## Markdown retirement rules

- Do not keep the same operational rule in multiple active documents.
- `PROGRESS.md` must not accumulate old logs.
- Major problem causes and reusable troubleshooting lessons belong in `docs/ops/INCIDENTS_AND_ROOT_CAUSES.md`.
- Studio visual confirmation rules belong in `docs/ops/STUDIO_CONFIRMATION_GATES.md`.
- Output-channel and parser semantics belong in `docs/febio/FEBIO_OUTPUT_MAPPING.md`.
- Stage-specific documents that are no longer active should move to history or legacy after their durable lessons are extracted.
- Temporary diagnostic documents must declare their retirement condition.

## 文書

- [README.md](../README.md)
  - 人間向け入口、概要、最短導線、最低限のコマンド。
- [AGENT.md](../AGENT.md)
  - AI / Codex 向けの不変運用ルール。
- [PROGRESS.md](../PROGRESS.md)
  - 現在状態と中断再開の単一ソース。
- [docs/ops/ROADMAP.md](ops/ROADMAP.md)
  - 全体優先順位、主ロードマップ、補助ロードマップ、stage 計画の単一ソース。直近の next action は置かない。
- [TASK_REQUEST_TEMPLATE.md](../TASK_REQUEST_TEMPLATE.md)
  - 日常作業依頼のテンプレ。
- [docs/DECISIONS.md](DECISIONS.md)
  - 設計判断の単一ファイル。
- [docs/VALIDATION_MATRIX.md](VALIDATION_MATRIX.md)
  - 変更種別ごとの最低検証。
- [docs/ops/](ops/)
  - 変更フロー、doc 同期、再開手順、Studio confirmation、incident/root-cause stock。
- [docs/research/](research/)
  - 研究問い、仮定、条件表、用語・指標。
- [docs/febio/](febio/)
  - active FEBio native spec、geometry conventions、output mapping、model refinement strategy、temporary active diagnostics。
- [docs/history/](history/)
  - historical progress and retired stage notes. Not active source-of-truth.

## テスト

- `tests/febio-front-end.test.mjs`
  - public API、FEBio export/import、classification、governance checks。
- `tests/load-app.mjs`
  - `generated/dist/public-api.js` の dynamic import helper。

## ビルドと入口

- `scripts/build-dist.mjs`
  - `src/**/*.ts` を `generated/dist/**/*.js` へコピーし、relative import extensions を書き換える。
- `scripts/export_febio_case.mjs`
  - legacy / compatibility canonical UI parameter path から FEBio handoff bundle を生成する。新しい solver behavior では触らない。
- `scripts/export_febio_direct_case.mjs`
  - legacy / compatibility S7-A/B/C direct FEBio handoff bundle generator。新しい solver behavior では触らない。
- `scripts/export_febio_native_case.mjs`
  - active native-only FEBio handoff bundle generator。`src/febio/native/index.ts` だけを入口にする。
- `scripts/dump_native_gmsh_baseline.mjs`
  - active Gmsh mesh workspace generator。default は `generated/gmsh_current/mesh.geo` / `mesh.msh` / validation artifacts。旧 `native-*` artifact は `--legacy-artifacts` 時だけ出す。
- `scripts/export_febio_from_gmsh_mesh.mjs`
  - active edited Gmsh `.msh` handoff generator。default export 先は `febio_exports/current_mesh/`。
- `scripts/export_febio_from_gmsh_python_api.mjs`
  - active generated Python API mesh bridge。default export 先は `febio_exports/current_mesh_api/`。
- `scripts/convert_febio_output.mjs`
  - legacy / compatibility FEBio logfile output converter。active export path では manifest に expected artifacts として提示するだけで呼ばない。
- `index.html`
  - `generated/dist/browser/main.js` を読み、legacy browser scripts を順に起動する。

## Legacy Directory

- `legacy/docs/febio/`
  - retired UI / canonical bridge docs and old parameter mapping docs.
- `legacy/febio_exports/`
  - historical S7-A/B/C export and run artifacts.

## 探索時の注意

- source-of-truth は `src/` を優先する。
- skill や AGENT rule が対象ファイルを示している場合、repo-wide exploration は避ける。
- public `index.ts` がある場合、internal file への deep import は避ける。
- `generated/` と `tmp/` は durable source ではない。
- 2026-04-24 note: FEBio export / conversion scripts use `src/public-api.ts` directly and do not read legacy JS simulation files. Browser compatibility scripts remain a UI bridge only; they are not source-of-truth for classification, detachment, export, or import.
