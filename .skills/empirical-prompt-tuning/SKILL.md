...
- Evaluate not only whether the task succeeds, but also whether it stayed within the intended exploration budget.
- From each empirical evaluation, classify the improvement target and propose updates accordingly:
  - `AGENT.md` for repo-wide operating rules and recurring governance issues
  - the relevant `SKILL.md` for missing local procedure, read-order, or allowed-reference constraints
  - `PROGRESS.md` for mismatched state recognition, implemented / partial / planned drift, or research-priority drift
- If exploration drift happened because `Read first`, `Allowed references`, or `Do not read first` was insufficient, call that out explicitly as a skill update candidate.
